...
export default function SellerDisputeDetailPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Ensure seller owns this dispute
      ...
      if (data.status === "resolved_refunded") {
        ...
        rSnap.forEach((doc) => setRefund(doc.data()));
      }
    ...
    <div className="space-y-10">
      ...
      <div className="bg-white p-6 rounded-xl shadow border space-y-4">
        ...
        <p className="text-gray-700">
          Buyer:{" "}
          <span className="font-semibold">{dispute.buyerId}</span>
        </p>
      ...
      {dispute.status === "resolved_refunded" && (
        <div className="bg-white p-6 rounded-xl shadow border space-y-4">
          <h2 className="text-xl font-semibold text-gray-900">Refund Outcome</h2>
