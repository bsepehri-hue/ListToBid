...
export default function PublicHomeDetailPage() {
  ...
  if (loading) {
    return <p className="text-gray-600">Loading home…</p>;
  }

  if (!listing) {
    return <p className="text-gray-600">Home not found.</p>;
  }
    ...
    <div className="space-y-10">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white p-6 rounded-xl shadow border">
          ...
          <Spec label="Home Type" value={listing.type} />
          <Spec label="Year Built" value={listing.yearBuilt} />
          <Spec label="Parking" value={listing.parking} />
          <Spec label="HOA Fee" value={listing.hoa ? `$${listing.hoa}/mo` : "None"} />
        </div>
      </div>
