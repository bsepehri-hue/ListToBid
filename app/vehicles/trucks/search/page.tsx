...
export default function TruckSearchPage() {
  ...
  const applyFilters = (filters: any) => {
    ...
    // Bed Length
    if (filters.bed !== "all") {
      results = results.filter((l) => l.bed === filters.bed);
    }

    // Cab Type
    if (filters.cab !== "all") {
      results = results.filter((l) => l.cab === filters.cab);
    }

    // Towing Capacity
    if (filters.tow !== "all") {
      if (filters.tow === "15000+") {
        results = results.filter((l) => l.tow >= 15000);
      } else if (filters.tow.startsWith("under")) {
        const max = Number(filters.tow.replace("under-", ""));
        results = results.filter((l) => l.tow <= max);
      } else {
        const [min, max] = filters.tow.split("-").map(Number);
        results = results.filter((l) => l.tow >= min && l.tow <= max);
      }
    }
  ...
  return (
    <div className="space-y-10">
      <h1 className="text-3xl font-bold text-gray-900">Search Trucks</h1>

      {/* Filters */}
      <TruckFilters onChange={applyFilters} />
      ...
      <div className="space-y-4">
        ...
        {filtered.length === 0 ? (
          <p className="text-gray-600">No trucks match your filters.</p>
        ) : (
