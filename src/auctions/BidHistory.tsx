      ...
      {bids.length === 0 ? (
        ...
        <div className="space-y-3">
          {bids.map((bid, index) => (
            <div 
              key={index} 
              className={`flex justify-between items-center py-2 ${index > 0 ? 'border-t border-gray-100' : ''}`}
            >
              {/* Bidder & Time */}
              <div className="flex items-center space-x-2">
                ...
                <span className="font-medium text-gray-700">
                  ...
                  {index === 0 && <span className="ml-2 text-xs font-bold text-teal-600">(Highest)</span>}
                </span>
              </div>
