...
import { ArrowUpRight, ArrowDownLeft, Ban, CreditCard, ExternalLink } from 'lucide-react';
import { Transaction } from '@/lib/mockData/vault';
...
const getTransactionMetadata = (type: Transaction['type'], amount: bigint) => {
...
export const TransactionRow: React.FC<TransactionRowProps> = ({ transaction }) => {
  ...
  const displayAmount = formatEther(amount > BigInt(0) ? amount : amount * BigInt(-1));
  ...
  return (
    <div className="grid grid-cols-12 gap-4 py-4 px-2 border-b border-gray-100 hover:bg-gray-50 transition duration-150">
      ...
      <div className="col-span-3 md:col-span-2 text-right">
        <p className={`font-semibold ${amount >= BigInt(0) ? 'text-green-700' : 'text-red-700'}`}>
          {sign} {displayAmount}
        </p>
